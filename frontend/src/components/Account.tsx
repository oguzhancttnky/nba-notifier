...
const Account: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const jwtToken = localStorage.getItem('jwtToken');
                await axios.put(`${process.env.REACT_APP_API_URL}/api/update/user/${userID}`, { email: values.email, password: values.password, chat_id: values.chatID }, {
                    headers: {
                        Authorization: `Bearer ${jwtToken}`
                    }
                });
        ...
        <div className="mx-auto mt-5">
            ...
            focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40`}
                            ...
                            {formik.touched.chatID && formik.errors.chatID ? (
                                <div className="text-red-500 text-sm mt-1">{formik.errors.chatID}</div>
                            ) : null}
                        </div>
                        <div className="relative mt-6">
                            <div className="flex items-center">
